...
- **연차/반차/반반차 use custom emoji** (`:yeoncha:`/`:bancha:`/`:banbancha:`)
  — bold text with a white sticker-style outline, generated by
  `scripts/text-emoji.ps1` and uploaded to the EverTri workspace. Slack shows
  status emoji at ~16-20px, so the outline is stamped by offsetting the same
  text rather than adding a padded badge shape, keeping the letters near
  full-canvas size — see `config/emoji/README.md` for what didn't work.
  Custom emoji are workspace-local — moving to a different Slack workspace
  means re-uploading them there first.
