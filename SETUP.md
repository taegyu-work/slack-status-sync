...
4. Roll out — send staff the Worker URL:
   ...
   > 공유일정 캘린더(경영기획팀 HR일정)의 일정(재택·연차·반차·반반차·외근)과 회의 일정에 맞춰 Slack 상태가
   > 자동으로 표시됩니다 🏠🌴☕🚗📅. 아래 링크에서 **본인 Slack 계정을 한 번만
   > 연결**해주세요. (이메일 읽기 권한 승인 필요)
   > 👉 https://<name>.<sub>.workers.dev
   > 직접 설정한 상태는 건드리지 않습니다.
   >
   > 📌 **외근은 공유일정 캘린더(경영기획팀 HR일정)의 일정만 인식합니다** — 개인
   > Outlook 캘린더에만 적어두시면 반영되지 않으니, 공유일정 캘린더(경영기획팀
   > HR일정)에 일정을 등록하시고 **제목에 `외근`이라는 단어를 꼭 넣어주세요** (제목 맨 앞에
   > `[외근]`을 붙이는 걸 권장합니다).
